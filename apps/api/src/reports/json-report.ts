...
export interface BuildReportFromAnalysisParams {
  gameId: string
}

export async function buildJsonReportFromAnalysis(
  params: BuildReportFromAnalysisParams,
  ...
  if (!game.rtpAnalysisJson) {
    throw new Error(`game ${gameId} has no completed RTP analysis`)
  }
  ...
  const analysis = game.rtpAnalysisJson as unknown as RtpAnalysis
  ...
  // Use the first (or only) variant for the top-level RTP values
  const primary = analysis.variants[0]
  const declaredRtp = primary?.declaredRtp ?? (game.declaredRtp ?? null)

  // Derive verdict from AI analysis: FAIL if delta > 0.1% from declared
  const verdictReasons: string[] = []
  if (schemaErrors.length > 0) {
    verdictReasons.push(`schema validation failed: ${schemaErrors.length} error(s)`)
  }
  for (const v of analysis.variants) {
    if (v.declaredRtp != null) {
      const delta = Math.abs(v.totalRtp - v.declaredRtp)
      if (delta > 0.001) {
        verdictReasons.push(
          `variant ${v.variantLabel}: computed RTP ${(v.totalRtp * 100).toFixed(3)}% deviates from declared ${(v.declaredRtp * 100).toFixed(3)}% by ${(delta * 100).toFixed(3)}%`,
        )
      }
    }
  }
  const verdict: import('./types.js').Verdict = verdictReasons.length > 0 ? 'FAIL' : 'PASS'
  if (verdict === 'PASS' && declaredRtp != null) {
    verdictReasons.push(`declared RTP ${(declaredRtp * 100).toFixed(2)}% verified within ±0.1%`)
  }
  if (verdict === 'PASS' && verdictReasons.length === 0) {
    verdictReasons.push('all checks passed')
  }
  ...
  // Build a synthetic simulation section from AI analysis data
  const totalRtp = primary?.totalRtp ?? 0
  const baseRtp = primary?.baseRtp ?? 0
  const freeSpinsRtp = primary?.freeSpinsRtp ?? 0
  const buyBonusRtp = primary?.buyBonusRtp ?? 0
  ...
  const report: GameReport = {
    ...
    overview: {
      ...
      gameType: label(analysis.gameType || schema.gameType, 'extracted'),
    ...
    features: {
      wild: label(
        schema.wild ? { symbolId: schema.wild.symbolId, substitutesFor: schema.wild.substitutesFor, multiplier: schema.wild.multiplier } : null,
        schema.wild ? 'extracted' : 'warning',
      ),
      scatter: label(
        schema.scatter ? { symbolId: schema.scatter.symbolId, triggerCount: schema.scatter.triggerCount, awardType: schema.scatter.awardType } : null,
        schema.scatter ? 'extracted' : 'warning',
      ),
      freeSpins: label(
        schema.freeSpins ? { count: schema.freeSpins.count, multiplier: schema.freeSpins.multiplier, retrigger: schema.freeSpins.retrigger, retriggerCount: schema.freeSpins.retriggerCount } : null,
        schema.freeSpins ? 'extracted' : 'warning',
      ),
      bonus: label(
        schema.bonus ? { description: schema.bonus.description, triggerCondition: schema.bonus.triggerCondition } : null,
        schema.bonus ? 'extracted' : 'warning',
      ),
      buyBonus: label(
        schema.buyBonus ? { costMultiplier: schema.buyBonus.costMultiplier, entryPoint: schema.buyBonus.entryPoint } : null,
        schema.buyBonus ? 'extracted' : 'warning',
      ),
    },
    simulation: {
      config: label({ spinCount: 0, rows: 3, seed: 0, simulateBuyBonus: false }, 'ai-inferred', 'derived analytically, not from Monte Carlo simulation'),
      rtp: label(
        { total: totalRtp, base: baseRtp, freeSpins: freeSpinsRtp, bonus: 0, buyBonus: buyBonusRtp },
        'ai-inferred',
      ),
      statistics: label(
        {
          ...
          totalBet: 0,
          totalReturn: 0,
          hitRate: primary?.hitRate ?? 0,
          variance: 0,
          standardDeviation: 0,
          confidence90: { low: totalRtp, high: totalRtp, halfWidth: 0 },
          confidence95: { low: totalRtp, high: totalRtp, halfWidth: 0 },
          featureTriggerCount: 0,
          durationMs: 0,
        },
        'ai-inferred',
        'statistics not available for analytical computation',
      ),
      symbolHitProbabilities: label(
        { maxCount: 0, totalSpins: 0, bySymbol: [], scatterHits: [], scatterProbs: [], wildAssistedWins: 0, wildAssistRate: 0 },
        'ai-inferred',
        'symbol hit data not available for analytical computation',
      ),
      buyBonus: label(null, 'warning'),
    },
    warnings,
